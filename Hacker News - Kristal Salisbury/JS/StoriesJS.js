...
async function putOtherUsersStoriesOnPage() {
  console.debug("putOtherUsersStoriesOnPage");

  $allStoriesList.empty();

  const allStories = await StoryList.getStories();
  
  if (allStories.stories.length > 0) {
    for (let story of allStories.stories) {
      const $story = generateStoryMarkup(story);
      $allStoriesList.append($story);
    }
  } else {
    $allStoriesList.append("<h5>No stories available from other users!</h5>");
  }
